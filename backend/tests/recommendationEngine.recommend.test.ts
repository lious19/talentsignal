...
describe("POST /api/recommendation-engine/recommend", () => {
  it("returns candidates ranked by fit score, most relevant first, each with feedback: 'none'", async () => {
    ...
    expect(res.body.candidates.every((c: { feedback: string }) => c.feedback === "none")).toBe(true);
    // Rationale carries through unchanged from scoreCandidate() — the whole
    // point of reusing it rather than reinventing a recommender.
    expect(res.body.candidates[0].matchedSkills).toEqual(["react", "sql"]);
    expect(res.body.candidates[0].reasons.length).toBeGreaterThan(0);
  });
